...
- **スコアリング**: 基本点 + コンボボーナス（詳細は[スコアルール](#-スコアルール)を参照）
...
```
獲得スコア = 基本点（10点） + コンボボーナス（現在のコンボ数 × 2点）
```
...
- コンボが高いほど1問あたりの得点が上昇
- **コンボ3以上**で特別な視覚エフェクト：
  - スコア表示がカラフルなグラデーションに変化
  - スコアポップアップ時にパーティクル（⭐✨）が飛び散る
  - スパークルアニメーションが表示される

#### スコア最大化のコツ

1. **コンボを途切れさせない** - 慎重に正解を選ぶ
2. **速く解く** - 制限時間内により多くの問題を解く
3. **サバイバルモード** - 長く生き残るほど高得点のチャンス

#### 不正解時のペナルティ

- スコア減点なし
- コンボ数が0にリセット
- 次の問題から基本点（10点）からスタート
